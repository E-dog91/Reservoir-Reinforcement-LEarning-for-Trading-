...
class Model_gru_ridge(Base_model_rc_ridge):
    def __init__(self, input_dim, hidden_dim, output_dim,
        ...
        # initializes the hidden cell
        self.rec_cell = nn.GRU(input_size=self._input_dim, hidden_size=self._hidden_dim, batch_first=True)
        ...
        W_ih, W_hh, b_ih, b_hh = self._generate_matrices(self._w_generator, self._win_generator, self._wbias_generator,
                                                         self.dtype,
                                                         self._input_dim, self._hidden_dim, 3 * self._hidden_dim)
